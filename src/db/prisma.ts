 ...
 */
/*
 * Ожидание свободного соединения ограничено. Без предела `pg` ждет вечно:
 * пул выбран утекшими клиентами или база не отвечает на подключение —
 * и тик, ждущий соединения, стоит навсегда, не падая и не сообщая ничего.
 * Десять секунд для тика в одну секунду — уже авария, и лучше получить ее
 * ошибкой, которую видно в логе.
 */
const pool = new Pool({ connectionString: env.databaseUrl, connectionTimeoutMillis: 10_000 });
