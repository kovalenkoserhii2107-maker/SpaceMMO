...
/*
 * Как часто состояние уходит в базу. Двадцать секунд, а не десять: дольше
 * резидентные командиры пишутся вдвое реже, а окно потери при жестком
 * перезапуске остается меньше минуты — таймеры считаются от абсолютных меток
 * и не теряются, теряется только накопленная за это время добыча.
 */
...
/*
 * Сколько командир держится в памяти без сокетов.
 *
 * Было шестьдесят секунд, и для живого игрока этого хватало. Но бот решает
 * раз в 45–65 секунд, то есть выгружался ровно между заходами, и каждый заход
 * начинался с полного чтения командира — со всеми базами, кораблями, обороной
 * и очередями. Семеро ботов давали около девяти таких чтений в минуту,
 * и на удаленной базе это оказалось дороже памяти, которую экономила выгрузка:
 * бесплатный тариф Neon (5 ГБ трафика в месяц) выбирался за трое суток,
 * после чего база отказывала и мир вставал целиком.
 *
 * Десять минут держат бота в памяти поперек нескольких заходов подряд.
 * Памяти это стоит по одному загруженному игроку на бота — ровно та цена,
 * которую исходное решение и называло приемлемой.
 */
