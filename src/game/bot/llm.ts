 ...
 * Сколько ждем ответа.
 *
 * Двадцать пять секунд, а не двенадцать: сводка выросла — в нее добавились
 * биржа, дипломатия, обломки и журнал решений, — а ответ теперь несет еще
 * и поручения. На живом заходе двенадцати уже не хватало, и вызов обрывался
 * по таймауту, теряя весь ход модели. Бот при этом не зависает: пока идет
 * запрос, тактику ведет код.
 *
 * Тридцать пять, а не двадцать пять: замер трех подряд вызовов дал разброс
 * от 0.9 до 13.9 секунды на одном и том же запросе. Задержка у сервиса
 * плавающая, и таймаут должен покрывать всплеск, а не средний случай.
 */
const TIMEOUT_MS = 35_000;

/** Потолок на ответ: курс плюс до четырех поручений с обоснованиями. */
