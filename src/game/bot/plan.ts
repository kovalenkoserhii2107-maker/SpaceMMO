 ...
 *
 * План управляет медленными вещами: долями бюджета, порядком науки, составом
 * эскадры. Пересматривать их чаще, чем успеваешь по ним что-то построить,
 * незачем — бот начинает метаться.
 *
 * Два часа, а не полчаса. Полчаса стояло с тех пор, когда ботов было трое
 * и за ними наблюдали: тогда это давало 144 вызова в сутки при норме в 400.
 * Семеро на том же такте дают 336 — то есть вся дневная норма уходит
 * на расписание, и на события, ради которых модель здесь и нужна, не остается
 * ничего. Замерено на живом стенде: 377 из 400 к вечеру.
 *
 * Считать легко: вызовов в сутки примерно (24 / часы) × число ботов. При двух
 * часах семеро дают 84, и три четверти нормы остаются событиям и переписке.
 * Добавляя ботов, эту цифру надо пересчитывать.
 *
 * Этим же окном ограничивается список событий в сводке: модель видит то,
 * что случилось с прошлого плана.
 */
export const PLAN_TTL_MS = 2 * 3600_000;
