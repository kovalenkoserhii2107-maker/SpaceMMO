 ...
 *
 * Позже ставка поднята до 4.5% вместе со снижением отдачи фермы до шести:
 * живой стенд показал, что при 3% и девяти кран все равно обгонял сток
 * втрое (₴2.12 млн в час против ₴718 тыс.), масса росла, и деньги мира
 * скапливались у одного бота. Перебор при отдаче 6:
 *
 *     ставка   масса     ₴ на единицу   сделок за 7 сут   верфь
 *     4.0%     ₴18.3 млн     0.34            10 906         6.0
 *     4.5%     ₴24.6 млн     0.48            13 738         6.1
 *     5.0%     ₴27.0 млн     0.55            15 382         5.9
 *
 * Взято 4.5%: соседние точки хуже по разным причинам — при 4% денег так мало,
 * что торговля не разворачивается, при 5% проседает верфь. Цены седьмого дня
 * в перебор не входят вовсе: в тонком стакане они скачут немонотонно
 * (918 → 370 → 64 → 62 → 286 за руду), и настраиваться по ним нельзя.
 *
 ...
 */
const HUB_RENT_SHARE_PER_HOUR = 0.045;
