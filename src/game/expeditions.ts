...
export interface ExpeditionResult {
  ...
  /** Найденные ресурсы; руда и полимеры занимают трюмы. */
...
export function resolveExpedition(
  ...
  if (event === 'SILENCE') {
    return {
      outcome: 'SILENCE',
      loot: { ore: 0, polymers: 0, antimatter: 0 },
      pirates: null,
      battle: null,
      survivors: { ...ships },
...
function resolveFind(ships: ShipCounts, capacity: number, level: number, rng: Rng): ExpeditionResult {
  const kind = rollWeighted(
    [
      ...
      ['POLYMERS', 35],
      ['ANTIMATTER', 15 + level],
    ] as Array<['ORE' | 'POLYMERS' | 'ANTIMATTER', number]>,
    rng,
  );

  const loot = { ore: 0, polymers: 0, antimatter: 0 };
  ...
  if (kind === 'ORE') loot.ore = amount;
  else loot.polymers = amount;
  ...
  return {
    ...
    summary:
      amount > 0
        ? `Заброшенный груз: подняли на борт ${amount} ${kind === 'ORE' ? 'руды' : 'полимеров'}.`
...
function resolveAmbush(ships: ShipCounts, capacity: number, level: number, rng: Rng): ExpeditionResult {
  ...
  return {
    outcome: 'PIRATES_WON',
    loot: { ore: trophy, polymers: 0, antimatter: 0 },
    pirates,
    battle,
    survivors: { ...battle.attackerSurvivors },
    summary:
      `Засада пиратов отбита (${describePirates(pirates)}). ` +
      `Из трюмов противника подняли ${trophy} руды.`,
  };
}
